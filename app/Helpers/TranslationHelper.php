...
if (!function_exists('translations')) {
    /**
     * return translations if translation file exists for a locale
     *
     * @param $locale
     * @return array|mixed
     * @throws Exception
...
if (!function_exists('generateTranslationKey')) {
     ...
     * @return string
     */
    function generateTranslationKey($prefix, $text)
    {
        $slug = str_replace(' ', '_', strtolower($text));
        return $prefix . '.' . $slug;
    }
}
...
if (!function_exists('detectLanguage')) {
    ...
    {
        // Simple language detection - in real app would use external service
        if (preg_match('/[آ-ی]/', $text)) return 'ar';
        if (preg_match('/[а-я]/i', $text)) return 'ru';
        if (preg_match('/[一-龯]/', $text)) return 'zh';
        if (preg_match('/[ひらがなカタカナ]/', $text)) return 'ja';
...
if (!function_exists('formatLocalizedNumber')) {
     ...
     * @return string
     */
    function formatLocalizedNumber($number, $locale = 'en')
    {
