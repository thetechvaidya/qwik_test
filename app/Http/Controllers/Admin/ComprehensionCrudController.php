...
class ComprehensionCrudController extends Controller
{
    ...
    {
        try {
            $comprehension = ComprehensionPassage::with('questions:id,comprehension_passage_id')->findOrFail($id);

            DB::transaction(function () use ($comprehension) {
                Question::whereIn('id', $comprehension->questions->pluck('id'))
                    ->update([
