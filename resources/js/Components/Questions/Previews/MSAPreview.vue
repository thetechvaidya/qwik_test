<template>
    <div class="bg-white shadow px-4 py-5 border-b-4 border-gray-800 mb-6">
        <h5 class="inline-block bg-green-100 rounded-sm px-2 py-1 mb-4 text-xs leading-3 text-green-700">{{
            question.skill
        }}</h5>
        ...
        <div v-show="!collapse" :id="question.code + '_options'">
            <ul v-for="(option, index) in question.options" :key="index" class="q-option">
                <li class="relative flex items-center mb-3 cursor-pointer p-3 rounded-sm border border-gray-200">
                    <div class="h-6 w-6 flex rounded-full items-center justify-center bg-gray-100 text-sm">{{
                        index + 1
                    }}</div>
                    <div class="text-sm font-normal ltr:ml-4 rtl:mr-4" v-html="option.option"></div>
                    <div
                        v-if="question.correct_answer === index + 1"
                        class="absolute ltr:right-2 rtl:left-2 text-green-500"
                    >
                        <svg
                            class="w-6 h-6"
                            fill="none"
        ...
        <a
            class="qt-link-success text-sm mb-4"
            @click="collapse = !collapse"
            v-html="collapse ? __('View Options') : __('Hide Options')"
        ></a>
        <ul class="mt-5">
        ...
        <div class="mt-4 flex justify-between items-center">
            <h5 class="inline-block bg-gray-100 rounded-sm px-2 py-1 text-xs leading-3 text-gray-700">{{
                question.code
            }}</h5>
