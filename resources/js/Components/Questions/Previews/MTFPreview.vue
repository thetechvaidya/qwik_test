        ...
        <div v-show="!collapse" :id="question.code+'_options'">
            <ul v-for="(option, index) in question.options" class="q-option">
                <li class="flex items-center mb-3 cursor-pointer p-3 rounded-sm border border-gray-200">
                    <div class="h-6 w-6 text-sm flex rounded-full items-center justify-center bg-gray-100 ">{{ index+1 }}
                    <div class="text-sm font-normal mx-4" v-html="option.option">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 8l4 4m0 0l-4 4m4-4H3"></path></svg>
                    <div class="text-sm font-normal ltr:ml-4 rtl:mr-4" v-html="option.pair">
                </li>
            </ul>
        </div>
