<template>
    <div class="overflow-y-auto h-screen px-2">
        ...
        <form v-else class="my-6 w-11/12 mx-auto xl:w-full xl:mx-0" @submit.prevent="submitForm">
            ...
            <div class="w-full flex flex-col mb-6">
                <label for="skill" class="pb-2 font-semibold text-gray-800"
                    >{{ __('Skill') }}<span class="ml-1 text-red-400">*</span></label
                >
                <v-select
                    id="skill"
                ...
                <small v-if="errors.skill_id" id="skill-help" class="p-invalid">{{ errors.skill_id }}</small>
            </div>
            <div class="w-full flex flex-col mb-6">
...
export default {
    ...
    methods: {
        ...
        update() {
            this.formValidated = true
            this.$inertia.patch(route('topics.update', { topic: this.topicId }), this.form, {
                onSuccess: () => {
                    if (Object.keys(this.errors).length === 0) {
                        this.$emit('close', true)
                    }
        ...
        fetch() {
            if (this.editFlag) {
                ...
                _this.loading = true
                axios
                    .get(route('topics.edit', { topic: this.topicId }))
