<template>
    <div class="overflow-y-auto h-screen px-2">
        ...
        <form v-else class="my-6 w-11/12 mx-auto xl:w-full xl:mx-0" @submit.prevent="submitForm">
            ...
            <div class="w-full flex flex-col mb-6">
                <label for="section" class="pb-2 font-semibold text-gray-800"
                    >{{ __('Section') }}<span class="ml-1 text-red-400">*</span></label
                >
                <v-select
                    id="section"
                ...
                <small v-if="errors.section_id" id="section-help" class="p-invalid">{{ errors.section_id }}</small>
            </div>
...
export default {
    ...
    methods: {
        ...
        update() {
            this.formValidated = true
            this.$inertia.patch(route('skills.update', { skill: this.skillId }), this.form, {
                onSuccess: () => {
                    if (Object.keys(this.errors).length === 0) {
                        this.$emit('close', true)
                    }
        ...
        fetch() {
            if (this.editFlag) {
                ...
                _this.loading = true
                axios
                    .get(route('skills.edit', { skill: this.skillId }))
