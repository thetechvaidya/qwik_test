...
const submitForm = async () => {
    ...
    try {
            ...
            },
            {
                onSuccess: () => {
                    toast({
                        severity: 'success',
                        summary: __('Success'),
                        detail: __('Quiz type created successfully'),
                        life: 3000,
                    })
                    emit('close')
                },
                onError: e => {
                    errors.value = e
                    console.error('Quiz type creation failed:', e)
                    toast({
                        severity: 'error',
                        summary: __('Error'),
                        detail: __('Failed to create quiz type. Please check the form and try again.'),
                        life: 5000,
                    })
                },
                onFinish: () => {
        ...
        console.error('Quiz type creation error:', error)
        toast({
            severity: 'error',
            summary: __('Error'),
