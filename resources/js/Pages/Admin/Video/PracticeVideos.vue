            ...
            >
                <div class="mb-4 sm:mb-0 md:mb-0 lg:mb-0 xl:mb-0 lg:w-1/2">
                    ...
                    <p class="font-normal text-sm text-gray-600 dark:text-gray-100 mt-1"
                        >{{ subCategory.name }} {{ skill.name }} {{ __('Videos') }}</p
                    >
                </div>
...
id="tag" v-model="tagArray" multiple :options="tags" label="name"
                                ...
                                <div class="w-full flex items-center gap-2 my-8">
                                    ...
                                    <button type="button" class="w-full qt-btn qt-btn-success" @click="filterVideos">{{
                                        __('Search')
                                    }}</button>
                                </div>
                            </div>
                        ...
                        <div class="sm:w-8/12 w-full my-4">
                            <div class="my-2 p-4 border border-green-100 shadow-sm rounded-sm bg-green-50">
                                ...
                                <div class="flex flex-col sm:flex-row gap-2">
                                    <div
                                        ><a href="#" class="text-sm qt-link-success" @click="viewVideos()"
                                            >{{ __('View') }} {{ __('Videos') }}</a
                                        ></div
                                    >
                                    <div class="hidden sm:inline-block">|</div>
                                    <div
                                        ><a href="#" class="text-sm qt-link-success" @click="editVideos()"
                                            >{{ __('Add') }} {{ __('Videos') }}</a
                                        ></div
                                    >
                                </div>
...
class="qt-btn-sm" :class="[qEditFlag ? 'qt-btn-success' : 'qt-btn-danger', video.disabled || processing ? 'opacity-25': '']" ]"
                                        ...
                                        <div
                                            v-else-if="pagination && !(videos.length === pagination.total)"
                                            class="flex justify-center items-center"
                                        >
                                            <button class="qt-btn-success" @click="loadMoreVideos">{{
                                                __('Load More')
                                            }}</button>
                                        </div>
...
const title = computed(() => {
    return __('Video/ Practice Videos') + ' - ' + pageProps.general.app_name
})
