    ...
    <AdminLayout>
        <template #header>
            <h4 class="page-heading"
                >{{ quiz.title }} - {{ schedule !== null ? __('Schedule') : '' }} {{ __('Overall Report') }}</h4
            >
        ...
        <template #actions>
            <Link
                v-if="schedule !== null"
                :href="route('quizzes.detailed_report', { quiz: quiz.id, schedule: schedule.id })"
                class="qt-btn qt-btn-success"
            >
                {{ __('Detailed Report') }}
            </Link>
            <Link v-else :href="route('quizzes.detailed_report', { quiz: quiz.id })" class="qt-btn qt-btn-success">
                {{ __('Detailed Report') }}
            </Link>
        </template>
...
const title = computed(() => {
    return __('Quiz/ Overall Report') + ' - ' + pageProps.general.app_name
})
