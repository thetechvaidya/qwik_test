                                ...
                                <template #body="slotProps" v-else-if="col.field === 'actions'">
                                    <div class="py-2">
                                        <ActionsDropdown>
                                            <template #actions>
                                                <Link
                                                    :href="route('quiz_session_results', {quiz: slotProps.data.slug, session: slotProps.data.id})"
                                                    class="action-item"
                                                    >{{ __('Results') }}</Link
                                                >
...
const {
    ...
    resourceKeys: ['quizSessions'],
    routeName: 'quizzes.detailed_report',
    routeParams: { quiz: props.quiz.id, ...(props.schedule ? { schedule: props.schedule.id } : {}) },
    columns: [
        {
            field: 'user.name',
            header: __('User'),
...
const deleteSession = (sessionId) => {
    confirm({
        ...
        accept: () => {
            router.delete(route('quiz_sessions.destroy', sessionId), {
                onSuccess: () => {
                    toast.add({
                        severity: 'success',
                        summary: __('Success'),
