...
class ExamRepositoryImpl implements ExamRepository {
  ...
  @override
  Future<Either<Failure, List<Exam>>> getFeaturedExams({int limit = 10}) async {
    try {
      if (await _networkInfo.isConnected) {
        try {
          final remoteExams = await _remoteDataSource.getFeaturedExams(limit: limit);
          
          // Cache the results
          await _localDataSource.cacheExams(remoteExams, cacheKey: 'featured');
          
          final domainExams = remoteExams.map((model) => model.toEntity()).toList();
  ...
  @override
  Future<Either<Failure, List<Exam>>> getRecentExams({int limit = 10}) async {
    try {
      if (await _networkInfo.isConnected) {
        try {
          final remoteExams = await _remoteDataSource.getRecentExams(limit: limit);
          
          // Cache the results
          await _localDataSource.cacheExams(remoteExams, cacheKey: 'recent');
  ...
  @override
  Future<Either<Failure, List<Exam>>> getPopularExams({int limit = 10}) async {
    try {
      if (await _networkInfo.isConnected) {
        try {
          final remoteExams = await _remoteDataSource.getPopularExams(limit: limit);
          
          // Cache the results
          await _localDataSource.cacheExams(remoteExams, cacheKey: 'popular');
  ...
  @override
  Future<Either<Failure, UserExamProgress?>> getUserExamProgress(String examId) async {
    try {
      if (await _networkInfo.isConnected) {
        try {
          final remoteProgress = await _remoteDataSource.getUserExamProgress(examId);
          
          // Cache the result
          if (remoteProgress != null) {
            await _localDataSource.cacheUserExamProgress(remoteProgress);
          }
          
          return Right(remoteProgress?.toEntity());
        } catch (e) {
          // If remote fails, try to get from cache
          final cachedProgress = await _localDataSource.getCachedUserExamProgress(examId);
  ...
  Future<Either<Failure, UserExamProgress>> updateUserExamProgress(
    ...
    try {
      if (await _networkInfo.isConnected) {
        final remoteProgress = await _remoteDataSource.updateUserExamProgress(
          examId,
          progressData,
        );
        ...
        return const Left(NetworkFailure(message: 'Cannot update progress without internet connection'));
      }
    } catch (e) {
      return Left(_mapExceptionToFailure(e));
    }
